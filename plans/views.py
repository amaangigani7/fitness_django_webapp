...
stripe.api_key = 'sk_test_51Ix2KQSIS67DOTGAxz8yNJs4MEb6bbmdBaI19iuTgcHLb2UAQZPnzNikgxsElY2J2htYg61G9i3CTmAfJmoIeDea00cTOoQuLX'
...
def checkout(request):
    ...
    coupons = {'halloween': 32, 'welcome': 10}
    if request.method == 'POST':
        stripe_customer = stripe.Customer.create(email=request.user.email, source=request.POST['stripeToken'])
        plan = 'price_1IxD9HSIS67DOTGAgub8TTis'
        if request.POST['plan'] == 'yearly':
            plan = 'price_1IxD9GSIS67DOTGAwNgeDOvm'
        if request.POST['coupon'] in coupons:
            ...
            try:
                ...
                pass
            # if customer.membership and customer.cancel_at_period_end:
            #     pass
            # else:
        ...
        if request.method == 'GET' and 'coupon' in request.GET:
            if request.GET['coupon'].lower() in coupons:
                ...
                coupon_rupee = str(coupon_price)
                final_rupee = str(price)

        return render(request, 'plans/checkout.html', {'plan': plan, 'coupon':coupon, 'price':price, 'og_rupee': og_rupee,
        'coupon_rupee': coupon_rupee, 'final_rupee': final_rupee})
